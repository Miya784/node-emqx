...
const addClient = async (req, res) => {
  try {
    ...
    const user = await _userModel.User.findByPk(userId);
    if (!user) {
    ...
    const existingClient = await _userModel.Client.findOne({
      where: {
        userId: user.id,
    ...
    const createdClient = await _userModel.Client.create({
      userId: user.id,
      typeClient: newtypeClient,
      client: newclient
    });
...
const getClients = async (req, res) => {
  try {
    ...
    const clients = await _userModel.Client.findAll({
      where: {
        userId: user.id
      }
    });
